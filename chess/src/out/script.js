import { movePiece, movType, resetmovTpe } from "./moves.js";
import { isKingAttacked, isInDanger } from "./attacks.js";
...
function rmFromBoard(y, x) {
    if (bcap === null || wcap === null || board[y][x].piece === null)
        return;
    const img = document.createElement("img");
...
async function mgReachableCell(y, x) {
    if (cpiece.x === null || cpiece.y === null || cpiece.piece === null)
        return;
...
function manageClick(piece, y, x) {
    ...
    if (piece === null || (!turn && co) || (turn && !co) || phase != 0)
        return;
    phase++;
    setNotReachable();
    cpiece.piece = piece;
